<template>
  <div class="">
    ...
    <video-operations ref="videoOperations" :cartoon-id="getCartoonId" @refresh-list="fetchCartoonVideoData"/>

    <video-select-list  ref="videoSelectList" :cartoon-id="getCartoonId" @refresh-list="fetchCartoonVideoData"/>
  ...
  export default {
    ...
    methods: {
      ...
      fetchCartoonVideoData() {
        if (this.getCartoonId !== null) {
          this.listQuery.videoId = this.getCartoonId
          this.tableLoading = true
          getVideoList(this.listQuery).then((res) => {
            if (res.data.code === 0) {
              const items = res.data.video.list
