<template>
  <div class="distpicker-address-wrapper">
    <template v-if="type != 'mobile'">
      <el-select @change="getCities" v-model="currentProvince" :disabled="disabled || provinceDisabled">
      ...
      <template v-if="!onlyProvince">
        <el-select @change="getAreas" v-model="currentCity" :disabled="disabled || cityDisabled">
        ...
        <el-select v-if="!hideArea" v-model="currentArea" :disabled="disabled || areaDisabled">
      ...
      <div :class="addressHeader">
        <ul>
          <li :class="{ 'active': tab === 1}" @click="resetProvince">{ {  currentProvince && !staticPlaceholder ?
            currentProvince : placeholders.province }}
          </li>
          <template v-if="!onlyProvince">
            <li v-if="showCityTab" :class="{ 'active': tab === 2}" @click="resetCity">{ {  currentCity &&
              !staticPlaceholder ? currentCity : placeholders.city }}
            </li>
            <li v-if="showAreaTab && !hideArea" :class="{ 'active': tab === 3}">{ {  currentArea && !staticPlaceholder ?
              currentArea : placeholders.area }}
            </li>
          </template>
        </ul>
      </div>
  ...
  export default {
    ...
    watch: {
      ...
      province(value) {
        ...
        this.currentProvinceText = this.getCodeValue(this.currentProvince)
      ...
      city(value) {
        ...
        this.currentProvinceText = this.getCodeValue(this.currentCity)
      ...
      area(value) {
        ...
        this.currentProvinceText = this.getCodeValue(this.currentArea)
      }
    },
    methods: {
      setData(value, check = '') {
